...
public class ProductController {
    ...
    @GET
    public Response getAllProducts(@DefaultValue("1") @QueryParam("page") int pageNumber,@DefaultValue("20") @QueryParam("rpp") int recordsPerPage, @QueryParam("fields") String fields) {
        ...
        List<String> partialFields = null;
        if (fields != null) {
            partialFields = List.of(fields.split(","));
            nextUriBuilder.queryParam("fields", fields);
            selfUriBuilder.queryParam("fields", fields);
        }
        Link next = Link.fromUri(nextUriBuilder.build()).rel("next").build();
        Link self = Link.fromUri(selfUriBuilder.build()).rel("self").build();
        if (partialFields == null || partialFields.isEmpty()) {
            ...
            List<Object[]> products = productService.findAll(pageNumber, recordsPerPage, partialFields);
            return Response.ok().links(next, self).entity(products).build();
        }
    ...
    @PATCH
    @Consumes(MediaType.APPLICATION_JSON)
    public Response patchProduct(@Context SecurityContext sec, ProductModel productModel) {
        authorize(sec);
        if (productService.patch(productModel)) {
            return Response.ok().entity("Patch Successfully").build();
        } else {
            return Response.status(Response.Status.BAD_REQUEST).entity("Couldn't put").build();
        }
    }
